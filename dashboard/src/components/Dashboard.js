  ...
  useEffect(()=>{
    const getUserVerified = async ()=>{
        try {
          ...
          if(!success){
            window.location.href = "http://localhost:3001/login/?error=unauthorized" // if token found to be invalid, redirect to /login page and pass a query error=unauthorized so that on the login page, we can extract this string and if it's equal to unauthorised, we'll pop us a toastify msg            
          } else{
            setUser(user);
            setLoading(false);
          }
